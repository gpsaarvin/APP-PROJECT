...
public class DoctorLoginController {
    ...
    private void handleLogin() {
        ...
        try {
            User user = AuthenticationService.getInstance().authenticateUser(username, password, User.UserType.DOCTOR);
    ...
    private void handleRegisterNavigation() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/doctor/register.fxml"));
            Parent root = loader.load();
            Stage stage = (Stage) usernameField.getScene().getWindow();
            stage.setScene(new Scene(root));
        } catch (IOException e) {
